...
function DocumentReviewModal({
  isOpen, onClose, docUrl, docTitle, profName, isProcessing, onVerify
}: {
  isOpen: boolean; onClose: () => void; docUrl: string | null; docTitle: string;
  profName: string; isProcessing: boolean;
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <motion.div 
        ...
        className="relative flex flex-col w-full max-w-5xl h-[90vh] overflow-hidden rounded-2xl bg-white shadow-2xl"
      >
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between shrink-0 bg-slate-50">
          <div>
            <h2 className="text-xl font-black text-[#0F172A]">{docTitle}</h2>
            <p className="text-sm font-medium text-slate-500">Document for {profName}</p>
          </div>
        ...
        <div className="flex-1 bg-slate-100 p-2 sm:p-6 overflow-hidden relative">
          {/* Using iframe for PDFs or images natively */}
          <iframe src={docUrl} className="w-full h-full rounded-xl bg-white shadow-sm border border-slate-200" title={docTitle} />
        </div>
          ...
          {rejectMode ? (
            <div className="animate-in fade-in slide-in-from-bottom-2">
              ...
              <textarea
                ...
                className="w-full h-20 rounded-xl border border-slate-300 p-3 text-sm text-[#0F172A] outline-none transition-all focus:border-red-500 focus:ring-2 focus:ring-red-500/20 resize-none bg-white mb-3"
              />
              <div className="flex items-center justify-end gap-3">
                <button onClick={() => setRejectMode(false)} className="px-4 py-2 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-100 transition-colors">Cancel</button>
                <button 
                  onClick={() => {
                    onVerify("rejected", note || "Documents need review.");
                    onClose();
                  }} 
            ...
            <div className="flex items-center justify-end gap-3">
              <button 
                ...
                disabled={isProcessing} 
                className="px-6 py-2.5 rounded-xl border border-red-200 bg-white text-sm font-bold text-red-600 hover:bg-red-50 hover:border-red-300 transition-all disabled:opacity-70 flex items-center gap-2"
              >
                <XCircle className="w-4 h-4" /> Reject Professional
              </button>
              <button 
                onClick={() => {
                  onVerify("verified", "Approved by Medrova admin.");
                  onClose();
                }} 
                disabled={isProcessing} 
                className="px-8 py-2.5 rounded-xl bg-[#10B981] text-sm font-bold text-white shadow-md shadow-emerald-500/20 hover:bg-emerald-600 transition-all disabled:opacity-70 flex items-center gap-2"
              >
                {isProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />} Approve Professional
              </button>
            </div>
          )}
        </div>
...
export function AdminProfessionalTable({ professionals: initialData }: AdminProfessionalTableProps) {
  ...
  const [reviewDoc, setReviewDoc] = useState<{ url: string; title: string; profId: string; profName: string } | null>(null);
  ...
  async function verifyProfessional(profId: string, status: VerificationStatus, note?: string) {
    ...
    try {
      // It points to professionals API, wait, our old API used to be /api/admin/doctors/[id]/verify.
      // We should probably update that backend route too if we renamed it, or keep the old name internally?
      // Ah, the verify API is still /api/admin/doctors/[id]/verify! I should use that for now or update it.
      // Let's use /api/admin/doctors/[id]/verify for now, since we only moved the GET route. 
      // Actually, wait, the API is `app/api/admin/doctors/[id]/verify/route.ts`... let me check if that exists.
                ...
                >
                  ...
                  <div className="flex flex-col sm:flex-row xl:flex-col gap-6 xl:w-1/3 shrink-0">
                    <div className="flex-1">
                      <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Review Documents</p>
                      <div className="flex gap-2">
                        <DocButton 
                          href={prof.primary_cert_url || undefined} 
                          label="Primary" 
                          onClick={() => prof.primary_cert_url && setReviewDoc({ url: prof.primary_cert_url, title: "Primary Certificate", profId: prof.id, profName: prof.name })} 
                        />
                        <DocButton 
                          href={prof.degree_cert_url || undefined} 
                          label="Degree" 
                          onClick={() => prof.degree_cert_url && setReviewDoc({ url: prof.degree_cert_url, title: "Degree Certificate", profId: prof.id, profName: prof.name })} 
                        />
                        <DocButton 
                          href={prof.gov_id_url || undefined} 
                          label="Gov ID" 
                          onClick={() => prof.gov_id_url && setReviewDoc({ url: prof.gov_id_url, title: "Government ID", profId: prof.id, profName: prof.name })} 
                        />
                      </div>
                    </div>
      ...
      <AnimatePresence>
        {reviewDoc && (
          <DocumentReviewModal 
            ...
            docUrl={reviewDoc.url} 
            docTitle={reviewDoc.title}
            profName={reviewDoc.profName}
            isProcessing={loadingId === reviewDoc.profId}
            onVerify={(status, note) => verifyProfessional(reviewDoc.profId, status, note)}
          />
        )}
      </AnimatePresence>
...
function DocButton({ href, label, onClick }: { href?: string; label: string; onClick?: () => void }) {
  if (!href) {
    return (
