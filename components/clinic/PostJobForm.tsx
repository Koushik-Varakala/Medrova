...
import { BriefcaseMedical, Loader2, Stethoscope, Clock, IndianRupee, FileText, CheckCircle2, AlertCircle, Users, Mail, Phone } from "lucide-react";
...
const postJobSchema = z.object({
  ...
  contactPhone: z.string().min(10, "Enter a valid phone number").optional().or(z.literal(""))
});
...
export function PostJobForm() {
  ...
  async function onSubmit(values: PostJobValues) {
    ...
    try {
      ...
      router.push("/dashboard/clinic/jobs");
      router.refresh();
    } catch {
    ...
    <form className="mx-auto max-w-3xl space-y-8 pb-24" onSubmit={handleSubmit(onSubmit)}>
            ...
            <LocationPicker
              ...
              onChange={(loc) => {
                ...
                if (loc) {
                  ...
                  setValue("locationDisplayName", undefined);
                }
              }}
            />
          </div>
      ...
      <section className="rounded-2xl border border-[#E2E8F0] bg-white p-6 shadow-sm sm:p-8">
        <div className="mb-6 flex items-center gap-3 border-b border-[#E2E8F0] pb-4">
          ...
          <h3 className="text-lg font-bold text-[#0F172A]">Salary & Contact</h3>
        </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          ...
          />
          <Input 
            label="Contact Email (Visible to applicants)" 
            type="email" 
          ...
          />
          <Input 
            label="Contact Phone (Visible to applicants)" 
            type="tel" 
