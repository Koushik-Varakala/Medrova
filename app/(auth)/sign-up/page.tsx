...
type SignUpRole = "doctor" | "nurse" | "technician" | "clinic";
...
const roleOptions: RoleOption[] = [
  {
    ...
    icon: Stethoscope,
    selectedRing: "ring-[#1E40AF] border-[#1E40AF]",
  ...
  },
  {
    role: "nurse",
    title: "I'm a Nurse",
    subtitle: "Find nursing roles and locum shifts",
    icon: Heart,
    selectedRing: "ring-emerald-500 border-emerald-500",
  ...
  },
  {
    ...
    subtitle: "Find lab, radiology, and OT roles",
    icon: FlaskConical,
    selectedRing: "ring-purple-500 border-purple-500",
  ...
  },
  {
    role: "clinic",
    title: "I'm a Clinic",
    subtitle: "Hire verified healthcare professionals",
    icon: Building2,
    selectedRing: "ring-slate-500 border-slate-500",
    selectedBg: "bg-slate-50/50",
...
function getOnboardingPath(role: SignUpRole): string {
...
export default function SignUpPage() {
  ...
  const [confirmedRole, setConfirmedRole] = useState<SignUpRole>("doctor");
  ...
  async function onSubmit(values: SignUpValues) {
    ...
    try {
      ...
      if (error) {
        if (error.status === 429) {
          setFormError("Too many sign-up attempts. Use a different email or wait a few minutes.");
          return;
        }
        if (error.status === 422) {
          if (error.message?.toLowerCase().includes("already registered") || error.message?.toLowerCase().includes("already exists")) {
            ...
            setFormError(error.message || "Password is too weak. Use at least 8 characters with an uppercase letter and a number.");
          }
          return;
        }
      ...
      if (!data.session) {
        // Email confirmation required — persist role so auth callback can use it
        localStorage.setItem("pending_signup_role", values.role);
    ...
    <main className="flex min-h-screen bg-white">
      ...
      <div className="relative hidden w-1/2 flex-col justify-between overflow-hidden bg-[#0F172A] p-12 lg:flex">
        ...
        <div className="relative z-10">
          <Link href="/">
            <Image src="/logo.png" alt="Medrova Logo" width={160} height={50} className="object-contain brightness-0 invert" priority />
          </Link>
          <div className="mt-24 max-w-md">
            <h1 className="text-4xl font-bold tracking-tight text-white">Join Medrova today</h1>
            <p className="mt-4 text-lg text-slate-300">
              Start finding shifts or hiring verified healthcare professionals in Hyderabad
            </p>
            <div className="mt-12 space-y-6">
              {[
                "Free to join for doctors, nurses & technicians",
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <p className="max-w-[280px] text-sm font-medium leading-relaxed text-white">
        ...
        <div className="w-full max-w-[440px]">
          <div className="mb-10 text-center lg:hidden">
            <Link href="/" className="inline-block">
              <Image src="/logo.png" alt="Medrova Logo" width={140} height={45} className="object-contain" priority />
            </Link>
          </div>
            ...
            {awaitingConfirmation ? (
              <div className="space-y-6 text-center">
                ...
                <p className="mx-auto max-w-sm text-sm leading-relaxed text-slate-500">
                  We sent a confirmation link to your email address. Click it to verify your account, then sign in to complete your{" "}
                  <span className="font-semibold text-[#1E40AF] capitalize">{confirmedRole}</span> onboarding.
                </p>
                <Link
                  href="/sign-in"
                  className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#1E40AF] px-4 py-3.5 text-sm font-semibold text-white shadow-lg shadow-blue-900/20 transition-all hover:bg-[#1D4ED8]"
                >
                  Go to sign in
                </Link>
              </div>
              ...
              <>
                <div className="hidden lg:block mb-8">
                  <Image src="/logo.png" alt="Medrova Logo" width={120} height={40} className="object-contain" priority />
                </div>
                <h2 className="text-3xl font-bold tracking-tight text-[#0F172A]">Create your account</h2>
                <p className="mt-2 text-sm text-slate-500">Choose your role and complete onboarding in minutes</p>
                ...
                <form className="mt-8 space-y-5" onSubmit={handleSubmit(onSubmit)}>
                        ...
                        return (
                          <motion.button
                            ...
                            className={cn(
                              ...
                              isSelected
                                ? `${option.selectedBg} ${option.selectedRing} shadow-md ring-1`
                                : "border-slate-200 bg-white hover:border-slate-300 hover:shadow-md"
                            )}
                          >
                            <div className={cn("rounded-lg p-2.5", isSelected ? `${option.iconBg} ${option.iconColor}` : "bg-slate-100 text-slate-500")}>
                              <Icon className="h-5 w-5" />
                            </div>
                            <p className={cn("mt-3 text-sm font-bold", isSelected ? option.selectedText : "text-[#0F172A]")}>{option.title}</p>
                            <p className="mt-1 text-xs leading-relaxed text-slate-500">{option.subtitle}</p>
                            {isSelected && (
                              <div className={cn("absolute right-3 top-3 rounded-full p-0.5 text-white", option.iconBg.replace("bg-", "bg-").replace("100", "600"))}>
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="group relative mt-2 flex w-full items-center justify-center gap-2 overflow-hidden rounded-xl bg-[#1E40AF] px-4 py-3.5 text-sm font-semibold text-white shadow-lg shadow-blue-900/20 transition-all hover:bg-[#1D4ED8] disabled:pointer-events-none disabled:opacity-70"
